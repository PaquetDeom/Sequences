...
public class ProgressionPanelTitleProgression extends JPanel {
	...
	public ProgressionPanelTitleProgression(ProgressionPanel progressionPanel) {
		super();
	...
	public void affiche() {
		...
		add(new JLabel("Nom de la progression : "), new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
	...
	private JLabel getLabelVisible() {
		boolean a = MainFrame.getUniqInstance().getProgressionVersion().isVisible();
	...
	private JLabel getLabelReferentiel() {
		...
		return new JLabel(MainFrame.getUniqInstance().getProgressionVersion().getReferentiel().toString() + " | ");
	}

	private JLabel getLabelAuteur() {

		return new JLabel(MainFrame.getUniqInstance().getProgressionVersion().getAuteur() + " | ");

	}

	private JLabel getLabelVersion() {

		return new JLabel("N°" + MainFrame.getUniqInstance().getProgressionVersion().getnVersion() + " | ");

	}

	private JLabel getLabelNom() {

		if (MainFrame.getUniqInstance().getProgressionVersion() == null)
			return new JLabel(" | ");
		else
			return new JLabel(MainFrame.getUniqInstance().getProgressionVersion().getTitre() + " | ");
